[**@exteditor/ghostbird**](../README.md)

***

[@exteditor/ghostbird](../modules.md) / ghosttext-runner

# ghosttext-runner

## Classes

- [GhostTextRunner](classes/GhostTextRunner.md)

## Interfaces

- [IClientEditor](interfaces/IClientEditor.md)
- [IClientOptionsLoader](interfaces/IClientOptionsLoader.md)
- [IGhostTextConnector](interfaces/IGhostTextConnector.md)
- [IHeart](interfaces/IHeart.md)
- [IMessagePort](interfaces/IMessagePort.md)
- [IMessenger](interfaces/IMessenger.md)
- [ISession](interfaces/ISession.md)
- [IStatusIndicator](interfaces/IStatusIndicator.md)
- [MessagesFromBackground](interfaces/MessagesFromBackground.md)

## Type Aliases

- [ClientOptions](type-aliases/ClientOptions.md)
- [ClientStatus](type-aliases/ClientStatus.md)

## References

### ExternalEdit

Re-exports [ExternalEdit](../ghosttext-session/type-aliases/ExternalEdit.md)

***

### InternalEdit

Re-exports [InternalEdit](../ghosttext-session/type-aliases/InternalEdit.md)
